...
When the user hovers over either a node or a link in the Sankey visualization, a popup showing
various information about the nodes / links is displayed.

By default:

- the node popup shows the name of the node and the count of individuals associated with that node;
- and the link popup shows the source and target of the link, the number of individuals associated
with that link and the proportion of individuals associated with source and target that pass through
that link.

The content of these popups can be customized by passing a `node_template` or `link_template`
...
Here we add a template string for the link popup that displays the name of the source and target of
